...
class AppdataStorage(object):
    ...
    def update(self, app_id, user_id, ip_address, bucket, document,
        ...
        if key:
            ...
            raise RuntimeError('Nor keys nor filter were specified for update')
    ...
    def delete(self, app_id, user_id, ip_address, bucket,
        ...
        if key:
            ...
            raise RuntimeError('Nor keys nor filter were specified for delete')
    ...
    def is_document_exists(self, app_id, user_id, bucket, criteria=None):
         ...
         res_criteria = self._generate_criteria(app_id, user_id, bucket)
         if criteria:
             if type(criteria) is not dict:
                 raise RuntimeError("Criteria object must be dict type.")
             if  extf.KEY in criteria:
                 document_id = self._internal_id(app_id, user_id, bucket,
                                                 criteria[extf.KEY])
                 res_criteria[intf.ID] = document_id

                 del criteria[extf.KEY]  # it is harmful field when iterating in the for loop bellow
                 # as we have _id search criteria no use search by app_id, user_id and bucket
                 del res_criteria[intf.APP_ID]
                 del res_criteria[intf.USER_ID]
                 del res_criteria[intf.BUCKET]
